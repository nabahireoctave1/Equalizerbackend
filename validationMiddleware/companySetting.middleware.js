...
const validateInput= [
    ...
    body('setting.interest_percentage').trim().notEmpty().withMessage('Company profit interest required!').isNumeric({min:1}),
     body("setting.overdue")
    .notEmpty().withMessage('Overdue message required!').custom(escapeHtmlChar),
     body("setting.payment_frequency").notEmpty().withMessage('Please choose payment frequency!')
    .isIn(["Daily", "Weekly", "Monthly"]),
      body("setting.reminder")
    .notEmpty().withMessage('Reminder message required!').customSanitizer(escapeHtmlChar),
