...
const validateInput= [
    ...
    body('setting.interest_percentage').trim().notEmpty().withMessage('Company profit interest required !').isNumeric({min:1}),
     body("setting.overdue").notEmpty().withMessage('Overdue message required !').custom(escapeHtmlChar),
     body("setting.payment_frequency").notEmpty().withMessage('Please choose payment frequency !').isIn(["Daily", "Weekly", "Monthly"]),
      body("setting.reminder").notEmpty().withMessage('Reminder message required !').customSanitizer(escapeHtmlChar),
      body('setting.report_generetion_time').notEmpty().withMessage('Report generation time is required !').matches(/^([01]\d|2[0-3]):([0-5]\d)$/)
     .withMessage('Valid time format is HH:MM '),
    body('setting.officeId').optional({ nullable: true }).isNumeric().withMessage('Please choose office'),
